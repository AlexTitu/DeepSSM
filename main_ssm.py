...
import random
import numpy as np
from utils import AudioAnomalyDataset, stratified_split, collate_fn, load_dataset
...
if not os.path.exists(f'./dev_ssm_2/general'):
    os.makedirs(f'./dev_ssm_2/general')

models_dir = f'./dev_ssm_2/general'
...
dataset_source, dataset_target = load_dataset('E:/Disertatie/DCASE2024', ('dev', 'train'), extension='.wav')
...
dev_train_dataset = AudioAnomalyDataset((source_train, target_train), 8000, 4000, 3, standardize=True)
dev_val_dataset = AudioAnomalyDataset((source_val, target_val), 8000, 4000, 3, standardize=True)

# for 1 type of machine, 1000 train samples
print(f'Number of train Samples: {len(dev_train_dataset)}')
print(f'Number of val Samples: {len(dev_val_dataset)}')
...
valDataLoader = DataLoader(dev_val_dataset, batch_size=BATCH_SIZE, collate_fn=collate_fn)
...
prev_train_state = torch.load(f"{models_dir}/train_state_dict_deep_ssm.pt")
DeepStateModel.load_state_dict(prev_train_state['model_state_dict'])
...
training_ssm(models_dir, DeepStateModel, optimizer, H, EPOCHS, trainDataLoader, valDataLoader,
             dev_train_dataset, dev_val_dataset, trainSteps, valSteps, scheduler)
